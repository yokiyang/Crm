...
<body>
	...
	<br />
	<form method="post" action="${base }/permission/emp/selectname">
		姓名:<input type="text" name="name" /> <input class="select"
			type="submit" value="查询" />
	</form>
	<br />

	<form method="post" action="${base }/permission/emp/selecttitle">
		<select name="title">
	...
	<form method="post" action="${base }/permission/emp/selectsalary">
	...
	<form method="post" action="${base }/permission/emp/selecthiredate">
	...
	<br />
	<hr />
	<c:set var="url" value="${base }/permission/emp/list" />
	<div>
		<c:if test="${pageBean.pageNow!=1 }">
			<a href="${url }?pageNow=1&pageSize=${pageBean.pageSize }">首页</a>
			&nbsp;
			<a
				href="${url }?pageNow=${pageBean.pageNow-1 }&pageSize=${pageBean.pageSize}">上一页</a>
			&nbsp;
		</c:if>
		<c:choose>
		...
		<c:forEach var="p" begin="${begin }" end="${begin+2 }">
			<c:if test="${p<=pageBean.pageCount }">
				<a href="${url }?pageNow=${p}&pageSize=${pageBean.pageSize}">${p
					}</a>
			</c:if>
		</c:forEach>
		&nbsp;
		<c:if
			...
			<a
				href="${url }?pageNow=${pageBean.pageNow+1 }&pageSize=${pageBean.pageSize}">下一页</a>
			&nbsp;
			<a
				href="${url }?pageNow=${pageBean.pageCount}&pageSize=${pageBean.pageSize}">尾页</a>
			&nbsp;
		</c:if>
...
<script>
	function go() {
		...
		if (reg.test(count)) {
			if (count > pageCount) {
				...
				window.location = "${url}?pageSize=${pageBean.pageSize}&pageNow="
						+ count;
			}
		} else {
			alert("请您输入正整数!!!");
	...
	function to(obj) {
		var value = obj.value;
		var url = "${url}?pageNow=1&pageSize=" + value;
		window.location = url;
	}
	...
	function deleteEmp(empId, status) {
		if (confirm("确定要删除该数据嘛?")) {
			...
			var url = "${base}/permission/emp/delete";
			xmlHttp.open("POST", url, true);
			xmlHttp.setRequestHeader("Content-type",
					"application/x-www-form-urlencoded;charset=UTF-8");
			xmlHttp.send("id=" + empId);
	...
	function batchDelete() {
		if (confirm("确定要删除这些数据嘛?")) {
			...
			var xmlHttp = AjaxFunction();
			var url = "${base}/permission/emp/batchdelete";
			xmlHttp.open("POST", url, true);
			xmlHttp.setRequestHeader("Content-type",
					"application/x-www-form-urlencoded;charset=UTF-8");
		...
		if (flag) {
			...
			input.onkeydown = function(event) {
				...
				if (keyCode == 13) {
					...
					var url = "${base}/permission/emp/editSalary?id=" + empId
							+ "&salary=" + sal;
